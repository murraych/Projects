...
public class MyFunProgram {
	...
	public static void main(String[] args) {
		...
		Scanner input = new Scanner(System.in); // ctrl-shift-o will fix imports for you
		
		System.out.println("Enter in 5 prices all on one line (seperated by a space): ");
		String priceInput = input.nextLine();
		System.out.println("You entered: " + priceInput);
		
		String[] priceStrings = priceInput.split(" "); // This line splits the price variable into array elements
		...
		int discount = Integer.parseInt(input.nextLine()); //longhand below
//		String discStr = input.nextline();
//		int disc = Integer.parseInt(discStr);
	
		System.out.println("After " + discount + "% discount:");
		for (int i = 0 ; i < priceArray.length; i++) {
			double afterDiscount = priceArray[i] * (1 - (double)discount / 100);
